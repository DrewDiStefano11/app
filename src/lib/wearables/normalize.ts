...
export function convertUnit(
...
): { value: number; canonicalUnit: CanonicalUnit; error?: RejectionReason } {
  const u = fromUnit.toLowerCase();

  if (type === "weight") {
    if (u === "kg" || u === "kilogram" || u === "kilograms")
      return { value: value * 2.20462, canonicalUnit: "lb" };
    if (u === "lb" || u === "lbs" || u === "pound" || u === "pounds")
      return { value, canonicalUnit: "lb" };
    return { value, canonicalUnit: "unknown", error: "incompatible_unit" };
  }
  if (type === "temperature") {
    if (u === "f" || u === "fahrenheit")
      return { value: ((value - 32) * 5) / 9, canonicalUnit: "celsius" };
    if (u === "c" || u === "celsius") return { value, canonicalUnit: "celsius" };
    return { value, canonicalUnit: "unknown", error: "incompatible_unit" };
  }
  if (type === "distance" || type === "elevation") {
    if (u === "mi" || u === "mile" || u === "miles")
      return { value: value * 1609.34, canonicalUnit: "meters" };
    if (u === "km" || u === "kilometer" || u === "kilometers")
      return { value: value * 1000, canonicalUnit: "meters" };
    if (u === "m" || u === "meter" || u === "meters") return { value, canonicalUnit: "meters" };
    if (u === "ft" || u === "foot" || u === "feet")
      return { value: value * 0.3048, canonicalUnit: "meters" };
    return { value, canonicalUnit: "unknown", error: "incompatible_unit" };
  }
  if (type === "active_calories" || type === "total_calories") {
    if (u === "j" || u === "joule" || u === "joules")
      return { value: value / 4184, canonicalUnit: "kcal" };
    if (u === "kj" || u === "kilojoule") return { value: value / 4.184, canonicalUnit: "kcal" };
    if (u === "kcal" || u === "calorie" || u === "calories" || u === "cal")
      return { value, canonicalUnit: "kcal" };
    return { value, canonicalUnit: "unknown", error: "incompatible_unit" };
  }
  ...
  if (type === "heart_rate_variability") {
    if (u === "s" || u === "sec" || u === "seconds")
      return { value: value * 1000, canonicalUnit: "ms" };
  ...
  if (type === "sleep_session") {
    if (u === "s" || u === "sec" || u === "seconds")
      return { value: value * 1000, canonicalUnit: "ms" };
    if (u === "m" || u === "min" || u === "minutes")
      return { value: value * 60000, canonicalUnit: "ms" };
    if (u === "h" || u === "hr" || u === "hours")
      return { value: value * 3600000, canonicalUnit: "ms" };
    if (u === "ms" || u === "milliseconds") return { value, canonicalUnit: "ms" };
    return { value, canonicalUnit: "unknown", error: "incompatible_unit" };
  }
...
function normalizeSleepStages(
  payload: unknown,
  sessionStart: number,
  sessionEnd?: number,
): SleepSessionPayload | null {
...
function normalizeWorkout(payload: unknown): WorkoutSessionPayload | null {
  ...
  return {
    workoutType:
      typeof p.workoutType === "string" ? String(p.workoutType).toLowerCase() : "unknown",
    ...
    averageHeartRate:
      typeof p.averageHeartRate === "number" && p.averageHeartRate > 0
        ? p.averageHeartRate
        : undefined,
  };
}
...
export function normalizeRecord(
...
): { record?: NormalizedRecord; error?: RejectionReason; message?: string; warning?: string } {
  ...
  if (value !== undefined) {
    ...
    if (bounds === "below")
      return {
        error: "below_minimum",
        message: `Value ${value} is below physiological minimum for ${type}`,
      };
    if (bounds === "above")
      return {
        error: "above_maximum",
        message: `Value ${value} is above physiological maximum for ${type}`,
      };
  }
  ...
  if (type === "sleep_stages" || type === "sleep_session") {
    ...
    if (!sleepNorm && !value && type === "sleep_stages")
      return { error: "malformed_payload", message: "Invalid or missing sleep stages payload" };
    if (sleepNorm) {
      payload = sleepNorm;
    }
  ...
  if (value === undefined && payload === undefined) {
    return {
      error: "missing_value_and_payload",
      message: "Record must have a value or valid payload",
    };
  }
