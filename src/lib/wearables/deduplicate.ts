...
function areRecordsNearEqual(a: NormalizedRecord, b: NormalizedRecord): boolean {
  ...
  if (rangeA && rangeB) {
    ...
    if (
      Math.abs(rangeA.startMs - rangeB.startMs) > 600000 ||
      Math.abs(rangeA.endMs - rangeB.endMs) > 600000
    )
      return false;
...
export function evaluateDuplicate(
  ...
  // 0. Exact Identical ID (Handles idempotency, exact updates, and exact revocations)
  if (existing.id === incoming.id) {
    if (incoming.revoked) {
      return {
        ...
        reason: "Incoming record marks existing identical ID record as revoked",
      };
    }
    ...
    if (incomingUpdate > existingUpdate) {
      return {
        ...
        incomingId: incoming.id,
        droppedRecordIds: [existing.id],
        reason: "Incoming record is a newer version of the exact same ID",
      };
    } else {
      return {
        kind: "duplicate",
        ...
        reason: "Existing record is newer or identical version of the same ID",
      };
    }
  }
  ...
  if (
    incoming.metadata?.linkedProviderRecordId === existing.id ||
    existing.metadata?.linkedProviderRecordId === incoming.id
  ) {
    return {
      kind: "duplicate",
      type: "replace-existing",
  ...
  if (areRecordsNearEqual(existing, incoming)) {
    ...
    if (incomingPriority > existingPriority) {
      return {
        ...
        directProviderPreferenceApplied:
          incoming.provenance === "direct" && existing.provenance === "aggregator",
      };
      ...
      return {
        ...
        directProviderPreferenceApplied:
          existing.provenance === "direct" && incoming.provenance === "aggregator",
      };
    } else {
  ...
  if (
    ...
    (existing.type === "sleep_session" || existing.type === "workout_session")
  ) {
  ...
  if (
    existing.type === incoming.type &&
    existing.type !== "sleep_session" &&
    existing.type !== "workout_session"
  ) {
    if (existing.endAt && incoming.endAt) {
      if (incoming.startAt < existing.endAt && incoming.endAt > existing.startAt) {
        return {
          ...
          overlapMs: Math.max(
            0,
            Math.min(existing.endAt, incoming.endAt) - Math.max(existing.startAt, incoming.startAt),
          ),
    ...
    for (let i = 0; i < result.length; i++) {
      ...
      if (resolution) {
        if (resolution.kind === "conflict") {
          // Do not break early on conflict. A conflict means "these two records are conflicting",
          // but we still need to check if the incoming record is an exact duplicate of *another* record
          // in the result array (e.g. its own previous self).
          // If we never find a 'duplicate' resolution, we'll keep both.
