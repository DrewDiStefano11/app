...
import {
  createAiEstimateProvenance,
  createJarvisProvenance,
  markProvenanceEdited,
} from "../fitcore-data";
...
export const TOOL_SPECS: ToolSpec[] = [
  ...
  {
    name: "getDailyDecision",
    description:
      "Get today's structured training, nutrition, recovery, what-changed, confidence, provenance, and single next-action decision.",
    parameters: emptyParams,
  },
];
...
function actionKey(tool: string, args: Record<string, unknown>) {
  ...
  if (tool === "logDailyCheckIn")
    return `checkin:${[args.energy, args.soreness, args.stress, args.motivation].map(Number).join(":")}:${normalizeText(args.notes)}:${todayStart()}`;
...
const handlers: Record<ToolName, ToolHandler> = {
  ...
  logBodyWeight: (args, { set, state, confirmed }) => {
    ...
    const provenance = createJarvisProvenance({
      confidence: "high",
      confirmed,
      auditId,
      originalText,
    ...
    set((s) => ({
      ...
      bodyweightEntries: [
        ...s.bodyweightEntries,
        { id, weightLb, notes: textArg(args, "notes"), createdAt: Date.now(), provenance },
      ],
    }));
    pushAudit(set, {
      id: auditId,
  ...
  logDailyCheckIn: (args, { set, state, confirmed }) => {
    const dupeAudit = hasAuditKey(state, "logDailyCheckIn", args);
    if (dupeAudit) return duplicateResult(dupeAudit);
  ...
  logMeal: (args, { set, state, confirmed }) => {
    ...
    const entry: MealEntry = {
      ...
      originalText,
      assumptions,
      confirmed: provenance.confirmation === "confirmed",
      auditId,
      provenance,
      createdAt: Date.now(),
    };
  ...
  updateMeal: (args, { set, state, confirmed }) => {
    ...
    const patch = (args.patch ?? {}) as Partial<MealEntry>,
      auditId = uid(),
      editedAt = Date.now();
    const provenance = markProvenanceEdited(meal.provenance, {
      editedBy: confirmed ? "user" : "jarvis",
      editedAt,
      auditId,
    });
    set((s) => ({
      ...
      mealEntries: s.mealEntries.map((m) =>
        m.id === id
          ? {
              ...
              ...patch,
              source: "edited",
              confirmed: confirmed ? true : m.confirmed,
              provenance,
            }
          : m,
      ),
  ...
  updateDailyCheckIn: (args, { set, state, confirmed }) => {
    ...
    const patch = (args.patch ?? {}) as Partial<RecoveryCheckIn>,
      auditId = uid(),
      editedAt = Date.now();
    const provenance = markProvenanceEdited(last.provenance, {
      editedBy: confirmed ? "user" : "jarvis",
      editedAt,
      auditId,
    });
    set((s) => ({
      ...s,
      recoveryCheckIns: s.recoveryCheckIns.map((c) =>
        c.id === last.id ? { ...c, ...patch, provenance } : c,
      ),
    }));
    pushAudit(set, {
      id: auditId,
  ...
  updateWorkout: (args, { set, state, confirmed }) => {
    ...
    const patch = (args.patch ?? {}) as Partial<Workout>,
      auditId = uid(),
      editedAt = Date.now();
    const provenance = markProvenanceEdited(workout.provenance, {
      editedBy: confirmed ? "user" : "jarvis",
      editedAt,
      auditId,
    });
    set((s) => ({
      ...s,
      workouts: s.workouts.map((w) => (w.id === id ? { ...w, ...patch, provenance } : w)),
    }));
    pushAudit(set, {
      id: auditId,
      tool: "updateWorkout",
  ...
  logExerciseSet: (args, { set, state, confirmed }) => {
    const dupeAudit = hasAuditKey(state, "logExerciseSet", args);
    if (dupeAudit) return duplicateResult(dupeAudit);
    ...
    const exerciseId = findExerciseId(args.exerciseId ?? args.exerciseName),
      ...
      auditId = uid();
    const newSet = {
      ...normalizeSet({ ...args, id: setId }),
      provenance: createJarvisProvenance({ confidence: "high", confirmed, auditId }),
    };
    ...
    pushAudit(set, {
      ...
      patch: { workoutId: active.id, exerciseId, actionKey: actionKey("logExerciseSet", args) },
      createdAt: Date.now(),
    });
  ...
  updateExerciseSet: (args, { set, state, confirmed }) => {
    ...
    const auditId = uid(),
      editedAt = Date.now();
    const provenance = markProvenanceEdited(prev.provenance, {
      editedBy: confirmed ? "user" : "jarvis",
      editedAt,
      auditId,
    });
      ...
      activeWorkout: s.activeWorkout
        ? {
            ...
            exercises: s.activeWorkout.exercises.map((e) => ({
              ...e,
              sets: e.sets.map((st) => (st.id === setId ? { ...st, ...patch, provenance } : st)),
            })),
          }
        : null,
    }));
  ...
  deleteExerciseSet: (args, { set, state }) => {
    ...
    pushAudit(set, {
      ...
      patch: { prev, exerciseEntryId: ex.id, exerciseId: ex.exerciseId },
      createdAt: Date.now(),
    });
  ...
  logCardio: (args, { set, state, confirmed }) => {
    ...
    const id = uid(),
      auditId = uid(),
      entryConfidence = asConfidence(args.confidence),
      originalText = textArg(args, "originalText");
  ...
  updateActiveWorkout: (args, { set, state, confirmed }) => {
    ...
    set((s) => ({
      ...s,
      activeWorkout: s.activeWorkout ? { ...s.activeWorkout, ...patch, provenance } : null,
    }));
    pushAudit(set, {
      id: auditId,
  ...
  finishActiveWorkout: (args, { set, state, confirmed }) => {
    const dupeAudit = hasAuditKey(state, "finishActiveWorkout", args);
    if (dupeAudit) return duplicateResult(dupeAudit);
    if (!state.activeWorkout) return { ok: false, summary: "no active workout" };
    const auditId = uid();
    const w: Workout = {
    ...
    set((s) => ({
      ...s,
      activeWorkout: null,
      workouts: s.workouts.some((existing) => existing.id === w.id)
        ? s.workouts
        : [...s.workouts, w],
    }));
    pushAudit(set, {
  ...
  logWorkoutPainOrSoreness: (args, { set, state, confirmed }) => {
    const dupeAudit = hasAuditKey(state, "logWorkoutPainOrSoreness", args);
    if (dupeAudit) return duplicateResult(dupeAudit);
    const notes = String(args.notes ?? "").trim();
    if (!notes) return { ok: false, summary: "notes required" };
    const area = textArg(args, "area"),
    ...
    pushAudit(set, {
      ...
      entityIds: [checkInId],
      entityKind: "checkin",
      patch: { area, notes, severity, actionKey: actionKey("logWorkoutPainOrSoreness", args) },
      createdAt: Date.now(),
    });
...
export function runTool(name: string, args: Record<string, unknown>, ctx: RunContext): ToolResult {
  ...
  if (
    ["logMeal", "logWorkout", "logCardio"].includes(name) &&
    asConfidence(args.confidence) !== "high" &&
    !ctx.confirmed
  ) {
    return {
      ok: true,
...
export function undoAuditEntry(auditId: string, state: AppState, set: Updater): ToolResult {
  ...
  switch (entry.entityKind) {
    ...
    case "activeSetDelete": {
      const p = entry.patch as
        | { prev?: SetEntry; exerciseEntryId?: string; exerciseId?: string }
        | undefined;
      if (p?.prev && (p.exerciseEntryId || p.exerciseId))
        set((s) => ({
          ...s,
          activeWorkout: s.activeWorkout
            ? {
                ...s.activeWorkout,
                exercises: s.activeWorkout.exercises.map((e) =>
                  (p.exerciseEntryId ? e.id === p.exerciseEntryId : e.exerciseId === p.exerciseId)
                    ? { ...e, sets: [...e.sets, p.prev!] }
                    : e,
                ),
              }
            : null,
