...
const VALID_ITEM_KINDS = new Set<ConversationContextItemKind>([
  "session_summary",
  "active_context",
  "user_message",
  "assistant_message",
...
function deepCloneAndFreezeJson<T extends JsonValue | undefined>(value: T): T {
  ...
  if (Array.isArray(value)) {
    const arr = value.map((item) => deepCloneAndFreezeJson(item));
    return Object.freeze(arr) as T;
  }

  const obj: Record<string, JsonValue> = {};
  for (const [key, val] of Object.entries(value)) {
    obj[key] = deepCloneAndFreezeJson(val) as JsonValue;
  }
  return Object.freeze(obj) as T;
}

function cloneAndFreezeItem(item: ConversationContextItem): ConversationContextItem {
  const clonedMetadata = item.metadata
    ? deepCloneAndFreezeJson(item.metadata as unknown as JsonValue)
    : undefined;

  return Object.freeze({
    ...item,
    metadata: clonedMetadata as Readonly<Record<string, JsonValue>> | undefined,
  });
}
...
export function projectConversationContext(
  ...
  const clonedItems = originalItems.map(cloneAndFreezeItem);
  const sortedItems = [...clonedItems].sort((a, b) => a.sequence - b.sequence);
  ...
  return {
    ...
    context: Object.freeze({
      ...
      includedItemIds: Object.freeze(includedItemIds),
      omittedItemIds: Object.freeze(omittedItemIds),
      includedTurnIds: Object.freeze(includedTurnIds),
      omittedTurnIds: Object.freeze(omittedTurnIds),
...
function validateItems(
  ...
  for (const item of items) {
    if (!VALID_ITEM_KINDS.has(item.kind)) {
      return createInvalidResult("invalid_item_kind", "A conversation item has an invalid kind.");
    }
  }
