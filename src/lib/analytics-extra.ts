...
import { addCalendarDays, calendarDayDifference, calendarDaysWindow, dateRangeContains, last7DaysWindow, previous7DaysWindow, startOfDay } from "./analytics/date-time";
...
export function volumeSeries(state: AppState, days: number, bucket: Bucket = "day", now = Date.now()):
  ...
  for (let i = months - 1; i >= 0; i--) {
    ...
    const vol = state.workouts.filter(w => w.startedAt >= start && w.startedAt < end)
      .reduce((s, w) => s + workoutVolume(w), 0);
    const d = new Date(start);
    out.push({ label: d.toLocaleDateString(undefined, { month: "short" }), volume: Math.round(vol), ts: start });
  }
  return out;
}

export function volumeByMuscle(state: AppState, days: number, now = Date.now()): { name: MuscleGroup; volume: number }[] {
...
export function volumeByExercise(state: AppState, days: number, now = Date.now()): { name: string; volume: number; sets: number }[] {
...
export function volumeByDayOfWeek(state: AppState, days: number, now = Date.now()): { label: string; volume: number; sets: number }[] {
...
export function setsSeries(state: AppState, days: number, now = Date.now()): { label: string; sets: number; reps: number; ts: number }[] {
...
export function compareWindows(state: AppState, days: number, now = Date.now()): { current: number; previous: number; deltaPct: number } {
