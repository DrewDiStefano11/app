...
export function readinessScore(state: AppState): number {
  ...
  const checkAvg = check.length
    ? check.reduce((s, c) => s + ((c.energy + c.motivation + (6 - c.soreness) + (6 - c.stress)) / 20), 0) / check.length
    : 0.7;
...
export function weeklyVolumeSeries(state: AppState, days = 7): { day: string; volume: number }[] {
  ...
  for (let i = days - 1; i >= 0; i--) {
    ...
    out.push({ day: d.toLocaleDateString(undefined, { weekday: "short" }).slice(0, 1), volume: Math.round(vol) });
  }
  return out;
}
...
export function trainingStreak(state: AppState): number {
  ...
  while (days.has(d) || days.has(d - 1)) {
    if (days.has(d)) { streak++; d--; }
    else if (streak === 0) { d--; } // allow today off
    else break;
  }
  return streak;
}
