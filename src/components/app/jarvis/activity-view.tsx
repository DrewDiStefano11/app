  ...
  return (
    <section>
      ...
      <Card className="space-y-2">
        {state.jarvisAudit.length === 0 && (
          <p className="text-sm text-muted-foreground">
            Nothing yet — anything Jarvis logs, suggests, or changes will show here.
          </p>
        )}
        {items.map((a) => (
          <div
          ...
          >
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[10px] text-muted-foreground">
                ...
                {a.undone ? "undone" : a.status}
              </p>
              {a.originalText && (
                <p className="text-xs italic text-muted-foreground truncate">"{a.originalText}"</p>
              )}
            </div>
            {!a.undone && a.status === "logged" && (
              <button
                onClick={() => undoAuditEntry(a.id, state, set)}
        ...
        {state.jarvisAudit.length > 6 && (
          <button
            ...
            className="text-xs text-muted-foreground w-full text-center pt-1"
          >
