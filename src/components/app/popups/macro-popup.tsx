    ...
    if (view2 === "daily") {
      for (let i = range - 1; i >= 0; i--) {
        ...
        out.push({
          ...
          calories: Math.round(t.c),
          protein: Math.round(t.p),
          carbs: Math.round(t.ca),
          fat: Math.round(t.f),
        });
      }
    } else {
      ...
      for (let i = weeks - 1; i >= 0; i--) {
        ...
        out.push({
          label: `W${weeks - i}`,
          calories: Math.round(t.c / 7),
          protein: Math.round(t.p / 7),
          carbs: Math.round(t.ca / 7),
          fat: Math.round(t.f / 7),
        });
      }
    }
    return out;
...
function MacroRing({
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <span className="text-[10px] font-bold uppercase tracking-wider text-white/50 mt-1">
        {label}
      </span>
    </div>
  );
