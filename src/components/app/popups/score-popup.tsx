...
function buildInsight(score: number, parts: Part[], best: string): string {
  ...
  if (score >= 80) {
    return `Your FitCore Score is ${score}. ${top.label} is the highest contributor at ${top.value}; ${low.label.toLowerCase()} is lowest at ${low.value}. Keep ${best} volume steady.`;
  }
  if (score >= 60) {
    return `Your FitCore Score is ${score}. ${top.label} is highest at ${top.value}, while ${low.label.toLowerCase()} is the lowest recorded contributor at ${low.value}. Consider a moderate ${best} session.`;
  }
  if (score >= 40) {
    return `Your FitCore Score is ${score}. ${top.label} is highest at ${top.value}; ${low.label.toLowerCase()} is lowest at ${low.value}. More consistent logging will make the comparison clearer.`;
  }
  return `Your FitCore Score is ${score} with limited recent inputs. Log a workout, a meal, and a recovery check-in to add measured context. Start with ${best}.`;
}
...
function nextAction(parts: Part[], best: string): string {
  ...
  switch (low.id) {
    case "train":
      return `Consider a 45-minute ${best} session and log the completed work.`;
    case "nutri":
      return `Move toward your protein target with a protein-forward meal or snack.`;
    case "recov":
      return `Log tonight's sleep and complete a recovery check-in.`;
    case "prog":
      return `Review your main lift before deciding whether to add a working set.`;
    default:
      return `Train ${best} today and log the completed session.`;
  }
}
