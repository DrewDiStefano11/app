      ...
      <div className="space-y-4">
        ...
        <div className="tile p-4">
          ...
          <div className="flex items-baseline gap-2 mt-1">
            ...
            {compare && (
              <span
                className={`text-xs font-bold ml-auto ${cmp.deltaPct >= 0 ? "text-green-400" : "text-red-400"}`}
              >
                {cmp.deltaPct >= 0 ? "+" : ""}
                {cmp.deltaPct}% vs prev
              </span>
            )}
          </div>
        </div>
        ...
        {groupBy === "total" && (
          <button
            ...
            style={
              compare ? { background: "var(--section-soft)", color: "var(--section)" } : undefined
            }
          >
              ...
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  <Bar dataKey="volume" radius={[0, 4, 4, 0]}>
                    {byMuscle.map((_, i) => (
                      <Cell key={i} fill="var(--section)" fillOpacity={1 - i * 0.06} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
                    ...
                    <div key={d.name} className="mb-2.5">
                      <div className="flex justify-between text-[11px] mb-1">
                        ...
                        <span className="text-white/50 tabular-nums">
                          {Math.round((d.volume / 1000) * 10) / 10}k · {d.sets} sets
                        </span>
                      </div>
...
function Empty() {
  return (
    <div className="h-full flex items-center justify-center text-xs text-white/40">
      No data in this range. Log a workout or enable Demo Data.
    </div>
  );
}
