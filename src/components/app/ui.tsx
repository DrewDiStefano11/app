...
export function SectionCard({ children, className }: { children: ReactNode; className?: string }) {
  return (
    <div className={cn("section-gradient border border-border rounded-2xl p-5", className)}>
      {children}
    </div>
  );
}
...
export function PrimaryButton({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        "inline-flex items-center justify-center gap-2 px-5 py-3 rounded-xl font-semibold text-white shadow-lg disabled:opacity-50 disabled:cursor-not-allowed active:scale-[0.98] transition-transform",
        className,
      )}
      style={{
...
export function GhostButton({
  ...
  return (
    <button
      ...
      {...props}
      className={cn(
        "inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl font-medium border border-border bg-[var(--surface)] hover:bg-[var(--surface-2)] transition-colors disabled:opacity-50 disabled:cursor-not-allowed",
        className,
      )}
    >
      {children}
...
export function Input(props: React.InputHTMLAttributes<HTMLInputElement>) {
  return (
    <input
      {...props}
      className={cn(
        "w-full px-4 py-3 rounded-xl bg-[var(--surface-2)] border border-border outline-none focus:border-[var(--section)] transition-colors text-base",
        props.className,
      )}
    />
  );
...
export function Select(props: React.SelectHTMLAttributes<HTMLSelectElement>) {
  return (
    <select
      {...props}
      className={cn(
        "w-full px-4 py-3 rounded-xl bg-[var(--surface-2)] border border-border outline-none focus:border-[var(--section)] text-base",
        props.className,
      )}
    />
  );
...
export function Textarea(props: React.TextareaHTMLAttributes<HTMLTextAreaElement>) {
  return (
    <textarea
      {...props}
      className={cn(
        "w-full px-4 py-3 rounded-xl bg-[var(--surface-2)] border border-border outline-none focus:border-[var(--section)] text-base resize-none",
        props.className,
      )}
    />
  );
