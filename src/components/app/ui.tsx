...
export function Card({ children, className, onClick }: { children: ReactNode; className?: string; onClick?: () => void }) {
  return (
    <div onClick={onClick} className={cn("card-elev p-4 sm:p-[1.125rem]", onClick && "cursor-pointer press transition-[transform,border-color,background-color]", className)}>
      {children}
    </div>
  );
}
...
export function SectionCard({ children, className }: { children: ReactNode; className?: string }) {
  return <div className={cn("section-gradient border border-border rounded-[var(--radius-card)] p-5 shadow-[var(--shadow-card)]", className)}>{children}</div>;
}

export function StatCard({ label, value, sub, accent }: { label: string; value: string | number; sub?: string; accent?: boolean }) {
  return (
    <Card className={cn("metric-shell flex flex-col gap-1.5 p-4", accent && "ring-section")}>
...
export function ProgressBar({ value, max, color }: { value: number; max: number; color?: string }) {
  ...
  return (
    <div className="h-2 rounded-full overflow-hidden ring-1 ring-white/[0.04]" style={{ background: "var(--surface-2)" }}>
...
export function EmptyState({ icon, title, description, action }: { icon?: ReactNode; title: string; description?: string; action?: ReactNode }) {
  return (
    <div className="empty-shell flex flex-col items-center text-center gap-3 py-9 px-6">
      {icon && <div className="empty-state-graphic" style={{ background: "var(--section-soft)", color: "var(--section)" }}>{icon}</div>}
...
export function PrimaryButton({ children, onClick, disabled, className, type }: { children: ReactNode; onClick?: () => void; disabled?: boolean; className?: string; type?: "button" | "submit" }) {
  return (
    <button type={type ?? "button"} disabled={disabled} onClick={onClick}
      className={cn("inline-flex min-h-12 items-center justify-center gap-2 px-5 py-3 rounded-[var(--radius-small)] font-semibold text-white disabled:opacity-50 disabled:cursor-not-allowed press transition-[transform,filter,box-shadow] hover:brightness-105", className)}
...
export function GhostButton({ children, className, type = "button", ...props }: ButtonHTMLAttributes<HTMLButtonElement> & { children: ReactNode; className?: string }) {
  return (
    <button type={type} {...props} className={cn("inline-flex min-h-11 items-center justify-center gap-2 px-4 py-2.5 rounded-[var(--radius-small)] font-semibold border border-[var(--border-strong)] bg-white/[0.045] hover:bg-white/[0.075] transition-colors disabled:opacity-50 disabled:cursor-not-allowed press", className)}>
      {children}
    </button>
  );
}
...
export function Input(props: React.InputHTMLAttributes<HTMLInputElement>) {
  return <input {...props} className={cn("w-full px-4 py-3 rounded-[var(--radius-small)] bg-[var(--surface-2)] border border-border outline-none focus:border-[var(--section)] transition-[border-color,box-shadow] text-base", props.className)} />;
}

export function Select(props: React.SelectHTMLAttributes<HTMLSelectElement>) {
  return <select {...props} className={cn("w-full px-4 py-3 rounded-[var(--radius-small)] bg-[var(--surface-2)] border border-border outline-none focus:border-[var(--section)] text-base", props.className)} />;
}

export function Textarea(props: React.TextareaHTMLAttributes<HTMLTextAreaElement>) {
  return <textarea {...props} className={cn("w-full px-4 py-3 rounded-[var(--radius-small)] bg-[var(--surface-2)] border border-border outline-none focus:border-[var(--section)] text-base resize-none", props.className)} />;
}
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          ...
          <div className="mt-1 flex items-end gap-2">
            <strong className="text-[length:var(--type-stat)] leading-none tabular-nums">{value}</strong>
            {status && <span className="pb-1 text-xs font-semibold text-[var(--text-secondary)]">{status}</span>}
          </div>
...
export function ErrorState({
  ...
  return (
    <div className="warning-shell border-[color:color-mix(in_oklab,var(--danger)_28%,transparent)] p-5 text-center">
      <div className="mx-auto grid h-12 w-12 place-items-center rounded-2xl bg-red-500/10 text-red-300">!</div>
