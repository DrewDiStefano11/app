...
import type { Workout, WorkoutExercise, SetEntry, CustomExercise, Goal } from "@/lib/types";
import {
  Card,
  PrimaryButton,
  GhostButton,
...
export function ActiveWorkoutView() {
  ...
  const activeId = useMemo(() => w.exercises.find((e) => !e.completed)?.id ?? null, [w.exercises]);
  ...
  return (
    <div className="pb-32">
      ...
      <div className="px-4 mt-3 space-y-2.5">
          ...
          <EmptyState
            ...
            action={
              <div className="flex gap-2">
                <PrimaryButton onClick={() => setExercisePicker(true)}>
                  <Plus size={16} />
                  Add
                </PrimaryButton>
        ...
        {w.exercises.map((we, idx) => (
          <ExerciseCard
            ...
            previousWorkout={state.workouts
              .slice()
              .reverse()
              .find((pw) => pw.exercises.some((pe) => pe.exerciseId === we.exerciseId))}
            demoMode={state.demoMode}
            onToggle={() => toggleOpen(we.id)}
            onMove={(dir) => moveExercise(we.id, dir)}
            onDelete={() =>
              updateActive((w) => ({ ...w, exercises: w.exercises.filter((x) => x.id !== we.id) }))
            }
        ...
        {w.exercises.length > 0 && (
          <div className="grid grid-cols-2 gap-2 pt-1">
            <GhostButton onClick={() => setExercisePicker(true)}>
              <Plus size={16} />
              Exercise
            </GhostButton>
            <GhostButton onClick={() => setCustomOpen(true)}>
              <Sparkles size={14} />
              Custom
            </GhostButton>
          </div>
      ...
      {/* Sticky finish bar */}
      <div
        className="fixed left-0 right-0 bottom-0 z-20 px-4 pb-[max(16px,env(safe-area-inset-bottom))] pt-3"
        style={{ background: "linear-gradient(to top, var(--bg) 70%, transparent)" }}
      >
        <div className="max-w-[480px] mx-auto flex gap-2">
          <GhostButton className="flex-1" onClick={() => setConfirmCancel(true)}>
            Discard
          </GhostButton>
      ...
      <PlateCalculatorSheet open={!!plateFor} onClose={() => setPlateFor(null)} />
      ...
      <ConfirmDialog
        ...
        message="All sets and notes from this session will be lost. This can't be undone."
...
function ExerciseCard({
  ...
  const topSet = workingSets.reduce<SetEntry | null>(
    (best, s) => (s.weight && (!best || (s.weight ?? 0) > (best.weight ?? 0)) ? s : best),
    null,
  );

  const prevEx = previousWorkout?.exercises.find((pe) => pe.exerciseId === we.exerciseId);
  const realPrevSets =
    prevEx?.sets.filter((s) => s.weight != null && s.reps != null && s.modifier !== "warmup") ?? [];
  const prevSets =
    realPrevSets.length > 0
      ? realPrevSets
      : demoMode
        ? makeDemoPrevSets(we.exerciseId, we.sets.length)
        : [];
  ...
  const applyTagToSet = (setId: string) => {
    ...
    onChange((e) => ({
      ...e,
      sets: e.sets.map((s) =>
        s.id === setId ? { ...s, modifier: s.modifier === armedTag ? "normal" : armedTag } : s,
      ),
    }));
  };
    ...
    <div
      ...
      style={
        isActive
          ? {
              boxShadow:
                "0 0 0 2px var(--section), 0 10px 30px -10px color-mix(in oklab, var(--section) 50%, transparent)",
            }
          : undefined
      }
    >
        ...
        <button onClick={onToggle} className="flex-1 text-left min-w-0">
          ...
          <div className="flex items-center gap-2 mt-0.5">
            <p className="text-[11px] text-muted-foreground capitalize truncate">
              ...
              {we.completed &&
                ` · ${workingSets.length} sets · ${Math.round(volume)}lb · ${reps} reps`}
            ...
            {tags.length > 0 &&
              tags.map((t) => (
                <span
                  ...
                  style={{ background: `${MOD_COLORS[t]}22`, color: MOD_COLORS[t] }}
                >
                  {t}
                </span>
              ))}
        ...
        <div className="px-3 pb-3 border-t border-border/60 pt-3 space-y-3">
          ...
          {prevSets.length > 0 && (
            <div className="text-[11px] text-muted-foreground/70 px-1">
              <span className="uppercase tracking-wider mr-1">
                Last{realPrevSets.length === 0 ? " (sample)" : ""}:
              </span>
              {prevTop && (
                <span>
                  best {prevTop.weight}×{prevTop.reps}
                </span>
              )}
            ...
            <div className="flex gap-1.5 overflow-x-auto no-scrollbar -mx-1 px-1 pb-0.5">
              {MODS.map((m) => {
                ...
                return (
                  <button
                    key={m}
                    onClick={() => setArmedTag(armed ? null : m)}
                    className="shrink-0 text-[10px] uppercase font-bold tracking-wider px-2 py-1 rounded-md border transition-all"
                    style={
                      armed
                        ? {
                            background: MOD_COLORS[m],
                            borderColor: MOD_COLORS[m],
                            color: "white",
                            boxShadow: `0 0 0 2px ${MOD_COLORS[m]}55`,
                          }
                        : onExercise
                          ? {
                              background: `${MOD_COLORS[m]}22`,
                              borderColor: MOD_COLORS[m],
                              color: MOD_COLORS[m],
                            }
          ...
          <div className="space-y-1.5">
            ...
            {we.sets.map((st, i) => (
              <SetRow
                ...
                onDelete={() =>
                  onChange((e) => ({ ...e, sets: e.sets.filter((s) => s.id !== st.id) }))
                }
              />
            ))}
          </div>
          ...
          <div className="flex flex-wrap gap-2 pt-1">
            <GhostButton
              ...
              onClick={() =>
                onChange((e) => ({
                  ...e,
                  sets: [...e.sets, { id: uid(), modifier: "normal", completed: false }],
                }))
              }
            >
              <Plus size={14} />
          ...
          {notesOpen ? (
            <Textarea
              ...
              onBlur={() => {
                if (!we.notes) setNotesOpen(false);
              }}
              className="text-sm"
            />
          ) : (
          ...
          <button
            ...
            style={{
              background: we.completed ? "var(--surface-2)" : "var(--section)",
              color: we.completed ? "var(--muted-foreground)" : undefined,
            }}
          >
          ...
          {warmupSets.length > 0 && !we.completed && (
            <p className="text-[10px] text-muted-foreground/70 text-center">
              {warmupSets.length} warmup set{warmupSets.length === 1 ? "" : "s"} won't count toward
              working volume.
            </p>
          )}
        </div>
    ...
    <div>
      <div className="flex items-center gap-1.5">
              ...
              : done
                ...
                : armedTag
                  ? {
                      ...
                      color: MOD_COLORS[armedTag],
                      border: `1.5px dashed ${MOD_COLORS[armedTag]}`,
                    }
        ...
        <input
          type="number"
          inputMode="decimal"
          placeholder={prev?.weight != null ? String(prev.weight) : "lb"}
          value={st.weight ?? ""}
          onChange={(e) =>
            onChange({ weight: e.target.value ? Number(e.target.value) : undefined })
          }
          className="flex-1 min-w-0 px-4 py-3 rounded-xl bg-[var(--surface-2)] border border-border outline-none focus:border-[var(--section)] text-base tabular-nums"
        />
        <input
          type="number"
          inputMode="numeric"
          ...
          className="w-24 px-4 py-3 rounded-xl bg-[var(--surface-2)] border border-border outline-none focus:border-[var(--section)] text-base tabular-nums"
        />
      </div>
      {tagOpen && (
        <div className="flex flex-wrap gap-1 mt-1.5 pl-9">
          ...
          {MODS.map((mod) => (
            <button
              key={mod}
              onClick={() => {
                onChange({ modifier: mod });
                setTagOpen(false);
              }}
              className="text-[10px] uppercase font-bold px-2 py-1 rounded-md border"
              style={{
                background: `${MOD_COLORS[mod]}22`,
                borderColor: MOD_COLORS[mod],
...
function ExercisePicker({
  ...
  const all = useMemo(
    () => [
      ...customExercises.map((c) => ({ ...c, isCustom: true as const })),
      ...EXERCISES.map((e) => ({ ...e, isCustom: false as const })),
    ],
    [customExercises],
  );
      ...
      <div className="space-y-3">
            ...
            {ALL_MUSCLES.filter((m) => m !== primary).map((m) => {
              ...
              return (
                <Chip
                  key={m}
                  active={on}
                  onClick={() => setSecondary((s) => (on ? s.filter((x) => x !== m) : [...s, m]))}
                >
                  {m}
                </Chip>
              );
        ...
        <div className="grid grid-cols-2 gap-2">
          <div>
            ...
            <Select
              value={category}
              onChange={(e) => setCategory(e.target.value as CustomExercise["category"])}
            >
        ...
        <PrimaryButton onClick={save} disabled={!name.trim()} className="w-full">
          Add to workout
        </PrimaryButton>
  ...
  return (
    <BottomSheet open={open} onClose={onClose} title="Plate calculator">
      <div className="space-y-3">
        <div className="grid grid-cols-2 gap-2">
          <div>
            <Label>Target (lb)</Label>
            <Input inputMode="decimal" value={target} onChange={(e) => setTarget(e.target.value)} />
          </div>
          <div>
            <Label>Bar (lb)</Label>
            <Input inputMode="decimal" value={bar} onChange={(e) => setBar(e.target.value)} />
          </div>
        </div>
        <div className="p-4 rounded-2xl" style={{ background: "var(--surface-2)" }}>
          ...
          {plates.length === 0 ? (
            ...
            <div className="flex flex-wrap gap-1.5 mt-2">
              {plates.map((p, i) => (
                <span
                  key={i}
                  className="px-2.5 py-1 rounded-md text-sm font-bold tabular-nums"
          ...
          <p className="text-xs text-muted-foreground mt-3">
            Total loaded:{" "}
            <span className="tabular-nums font-semibold text-foreground">{total} lb</span>
          </p>
        </div>
        <div className="flex gap-2 flex-wrap">
          {[95, 135, 185, 225, 275, 315].map((v) => (
            <Chip key={v} active={t === v} onClick={() => setTarget(String(v))}>
              {v}
            </Chip>
          ))}
        </div>
...
function FinishWorkoutSheet({
  ...
  // Stats
  const workingSets =
    w?.exercises.flatMap((e) => e.sets.filter((s) => s.completed && s.modifier !== "warmup")) ?? [];
  const totalSets = workingSets.length;
  const totalVolume = Math.round(
    workingSets.reduce((a, s) => a + (s.weight ?? 0) * (s.reps ?? 0), 0),
  );
      ...
      for (const s of we.sets) {
        if (s.completed && s.weight && s.reps && s.modifier !== "warmup") {
          ...
          if (!prev || est > prev.value) {
            ...
            if (existing >= 0) {
              if (out[existing].est < est) out[existing] = entry;
            } else out.push(entry);
          }
        }
      }
  ...
  const confirmSave = () => {
    ...
    const goals: Goal[] = state.goals.map((g) =>
      g.type === "weekly_workouts" ? { ...g, current: Math.min(g.target, g.current + 1) } : g,
    );
    ...
    <BottomSheet open={open} onClose={onClose} title="Finish workout" height="tall">
      <div className="space-y-4">
        ...
        <div>
          ...
          <Textarea
            rows={3}
            placeholder="How did it feel? Pain, energy, sleep…"
            value={notes}
        ...
        <div>
          ...
          <div className="mt-1 space-y-1.5">
            <SaveOption
              ...
              desc="Add to history, update stats and PRs."
            />
            {w.templateId && (
              <SaveOption
                ...
                desc="Apply changes to the original template."
              />
            )}
            <SaveOption
              value="save_as_new"
              ...
              desc="Reuse this workout layout later."
            />
          </div>
...
function Stat({
  ...
  return (
    <div
      ...
      style={{ background: accent ? "var(--section-soft)" : "var(--surface-2)" }}
    >
      <p className="text-[9px] uppercase tracking-wider text-muted-foreground">{label}</p>
      <p
        className={`text-base font-bold tabular-nums ${mono ? "font-mono" : ""}`}
...
function extractNoteSuggestions(
  text: string,
): { muscle?: string; level: "fresh" | "moderate" | "fatigued" | "very"; summary: string }[] {
  if (!text) return [];
  const out: {
    muscle?: string;
    level: "fresh" | "moderate" | "fatigued" | "very";
    summary: string;
  }[] = [];
  const lower = text.toLowerCase();
  const heavySignals = /(pain|hurt|injur|tight|very sore|extremely sore)/i.test(text);
  const moderateSignals = /(sore|tired|fatigue|low energy|bad sleep|poor sleep|exhausted)/i.test(
    text,
  );
