...
type HubCardProps = {
  ...
  icon?: ReactNode;
  expanded: boolean;
  onToggle: (id: HubSectionId) => void;
  children: ReactNode;
};

function StatusChip({ children, tone = "default" }: { children: ReactNode; tone?: "default" | "active" | "planned" | "danger" }) {
  ...
  return (
    <span className={`inline-flex min-h-7 items-center rounded-full border px-2.5 py-1 text-[11px] font-semibold leading-none ${toneClass}`}>
      {children}
    </span>
  );
}
      ...
      >
        ...
        <span className="min-w-0 flex-1">
          <span className="flex flex-wrap items-center gap-2">
            ...
            {status && <StatusChip tone={status === "Active" ? "active" : status === "Destructive" ? "danger" : "planned"}>{status}</StatusChip>}
          </span>
...
function FutureRows({ rows }: { rows: { label: string; detail: string; status: string }[] }) {
  return (
    <div className="space-y-2">
      {rows.map(row => (
        <div key={row.label} className="flex flex-col gap-2 rounded-2xl border border-border bg-[var(--surface-2)] p-3 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div className="mx-auto max-w-3xl px-5">
        <section className="rounded-[var(--radius-card)] border border-border bg-[linear-gradient(135deg,var(--surface-2),transparent)] p-5 shadow-[var(--shadow-card)] sm:p-6">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div className="min-w-0">
              <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">FitCore control center</p>
              <h2 className="mt-2 text-2xl font-bold leading-tight sm:text-3xl">Settings that feel ready for today and organized for what comes next.</h2>
              <p className="mt-3 max-w-2xl text-sm leading-relaxed text-muted-foreground">
                Tune profile targets, manage Jarvis controls, review local data actions, and see planned attach points without turning them into live features.
              </p>
            </div>
        ...
        <div className="mt-6 space-y-4">
          <HubCard
            id="profile"
            title="Profile & Goals"
            description="Goal, experience, schedule, bodyweight, target weight, and units."
            status="Active"
            icon={<User size={18} />}
          ...
          <HubCard
            id="jarvis"
            title="Jarvis / AI"
            description="Existing Jarvis settings, goals profile, and activity history."
            status="Active"
            icon={<BrainCircuit size={18} />}
            expanded={expanded.jarvis}
            onToggle={toggleSection}
          >
            <div className="space-y-4">
          ...
          >
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <p className="text-xs leading-relaxed text-muted-foreground">Reminders use local browser notifications. The app functions normally without them.</p>
                {(["workout","weighIn","lunch"] as const).map(k => (
                  <label key={k} className="flex items-center justify-between gap-3 rounded-2xl border border-border bg-[var(--surface-2)] p-3">
          ...
          >
            <FutureRows rows={[
              { label: "Body metrics", detail: "A future home for measurements beyond bodyweight.", status: "Coming later" },
              { label: "Injuries", detail: "Planned context for training limitations and active pain areas.", status: "Not connected yet" },
              { label: "Medications/allergies", detail: "Reserved for user-entered profile notes only; no medical logic is active.", status: "Planned" },
              { label: "Recovery preferences", detail: "Future place for sleep, soreness, and readiness preferences.", status: "Coming later" },
              { label: "Training limitations", detail: "Reserved for constraints like equipment, movements, or schedule limits.", status: "Not connected yet" },
            ]} />
          </HubCard>
          ...
          >
            <FutureRows rows={[
              { label: "Local-only data", detail: "Current app data stays on this device unless you export it.", status: "Active" },
              { label: "AI memory controls", detail: "Existing Jarvis controls live in Jarvis / AI; broader privacy controls are planned.", status: "Planned" },
              { label: "Export/delete data", detail: "Current export, import, and reset actions are in Data Management below.", status: "Available" },
              { label: "Source visibility", detail: "Future area for showing which app data powers each feature.", status: "Coming later" },
            ]} />
          </HubCard>
          ...
          >
            <div className="space-y-4">
              <div>
                ...
                <p className="mt-1 text-xs leading-relaxed text-muted-foreground">Your data is stored locally. Use backups to secure your progress.</p>
              </div>
              ...
              <div className="rounded-2xl border border-destructive/35 bg-destructive/5 p-3">
                <p className="mb-3 text-xs font-semibold uppercase tracking-wider text-destructive">Destructive action</p>
          ...
          >
            <FutureRows rows={[
              { label: "Wearables", detail: "Reserved for watches and recovery trackers. No device connection is active.", status: "Not connected" },
              { label: "Apple Health / Google Fit", detail: "Future attach point for health platform sync.", status: "Planned" },
              { label: "Smart scale", detail: "Future place for bodyweight source connections.", status: "Coming later" },
              { label: "Food, camera, and macros", detail: "Reserved for nutrition capture integrations without adding logic here.", status: "Planned" },
            ]} />
          </HubCard>
          ...
          >
            <FutureRows rows={[
              { label: "Allergies", detail: "Future profile note area. No medical advice or alerts are active.", status: "Coming later" },
              { label: "Medications", detail: "Reserved for optional user-entered context in a later release.", status: "Planned" },
              { label: "Conditions and red flags", detail: "Future safety profile concept only.", status: "Not connected yet" },
              { label: "Emergency contacts", detail: "Placeholder for later contact information support.", status: "Planned" },
            ]} />
          </HubCard>
          ...
          >
            <FutureRows rows={[
              { label: "Coach view", detail: "Future area for sharing progress with a coach or trainer.", status: "Planned" },
              { label: "Gym or PT clinic", detail: "Reserved for business workflows without adding routes or billing.", status: "Coming later" },
              { label: "Premium / Pro", detail: "Presentational placeholder only; no subscription or payment logic is active.", status: "Not connected yet" },
            ]} />
          </HubCard>

          <HubCard
            id="about"
            title="About / Local App Info"
