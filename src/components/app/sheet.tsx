    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center">
      ...
      <div className={cn(
        ...
        <div className="flex min-h-16 items-center justify-between border-b border-white/[0.06] px-5 pb-3 pt-5">
          ...
          <button onClick={onClose} className="grid h-10 w-10 place-items-center rounded-full border border-white/10 bg-white/5 text-white/75 transition-colors hover:bg-white/10 hover:text-white press">
            <X size={18} />
          </button>
        </div>
        <div className="sheet-scroll overflow-y-auto px-5 pb-[max(24px,env(safe-area-inset-bottom))] pt-4" style={{ maxHeight: "calc(88dvh - 60px)" }}>
          {children}
        </div>
      </div>
    </div>
