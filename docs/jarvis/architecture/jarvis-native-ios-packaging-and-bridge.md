...
| -------------------- | ----------------------------------------------------- | ----------------------------------------------------------------------- | ---------- |
| **Framework**        | React 19, Vite, TanStack Start, Tailwind              | Capacitor wraps Vite outputs transparently.                             | High       |
| **Routing**          | TanStack Router (`src/router.tsx`)                    | Client-side routing maps cleanly to WKWebView.                          | High       |
| **Build Output**     | Vite build to `dist`/`build` (Nitro/Cloudflare)       | Output directory must be synced into Capacitor's web folder.            | High       |
...
| **Storage**          | LocalStorage/Atomic (`src/lib/atomic-persistence.ts`) | WKWebView LocalStorage is preserved but not guaranteed permanent.       | Medium     |
| **Service Worker**   | Basic PWA (`public/sw.js`)                            | Caching might conflict with local native bundles; may need disablement. | Medium     |
| **PWA Behavior**     | `public/manifest.json`                                | Replaced by native App Store packaging.                                 | High       |
| **Browser APIs**     | Standard web APIs                                     | Direct `navigator.mediaDevices` will be replaced by native iOS audio.   | High       |
...
- **Required native structure:** A standard Capacitor iOS project wrapping the dist output, plus a local Swift plugin module.
...
+-------------|-----------------------------|---------------+
              v                             v
     Hardware (Mic/Speaker)          Local Data (IndexedDB)
```
...
| `src/router.tsx`                | Minimal                     | Capacitor uses hash or memory routing if needed. | Medium | High       |
| `src/lib/store.tsx`             | High (Source of truth)      | Must sync state changes safely via bridge.       | Medium | High       |
| `src/lib/atomic-persistence.ts` | Medium                      | iOS LocalStorage limits/clearing behavior.       | Medium | Medium     |
| `public/sw.js`                  | Medium                      | Conflicts with native asset loading.             | Medium | Medium     |
| Vite build output               | High                        | Output must route to native `public/` folder.    | Low    | High       |
...
## 45. Final recommendation summary

- **Packaging:** Capacitor wraps the existing Vite application.
