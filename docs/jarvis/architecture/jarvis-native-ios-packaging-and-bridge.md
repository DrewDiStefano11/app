...
- **What moves to native Swift:** Microphone and audio-session ownership, approved native speech and inference providers, lifecycle handling, and model-asset management.
...
**Decision:** Capacitor is the preferred packaging feasibility candidate because it preserves most of the existing React interface. A physical-device spike must prove app loading, offline behavior, storage behavior, lifecycle handling, event streaming, cancellation, and bridge stability. Capacitor is not approved for production until the packaging feasibility gate passes. The custom WKWebView option remains the fallback. A frontend rewrite is not approved. Final selection depends on app loading, persistence, lifecycle, bridge throughput, cancellation, offline behavior, signing, and physical-device stability.
...
- **How updates are delivered:** Standard App Store updates. Web assets are bundled with the signed native application. Application behavior is updated through standard App Store releases. No remote application-code or live-update mechanism is included initially.
- **Are over-the-air web updates allowed?** No remote application-code or live-update mechanism is included initially. Any future remote web-asset update mechanism requires a separate reviewed decision covering: current Apple App Review policy, signing and integrity, rollback, service-worker behavior, native/web version compatibility, security, privacy, and behavior changes after review.
...
- **Reconsideration triggers:** Reconsideration of the framework will be based on total bridge overhead, time to first visible response, partial-transcript delivery, event ordering, cancellation latency, stale-event rejection, UI frame stability, main-thread blocking, payload batching, event backpressure, long streamed responses, tool request/result round trips, behavior during backgrounding and restoration, and physical iPhone testing.
...
- Method names and events are clearly labeled conceptual.
- Final methods are defined by a typed plugin contract.
- Request and response envelopes are versioned.
- Events include session ID, turn ID, sequence number, and protocol version.
...
- No canonical persistence API is exposed across the bridge.
...
- **Existing FitCore storage (`src/lib/store.tsx`):** Canonical FitCore persistence services own workout, nutrition, recovery, sleep, goal, and other domain records. Existing repository storage behavior must be tested inside WKWebView. Jarvis does not directly own canonical FitCore records. Packaging alone does not trigger a persistence migration.
...
- **Matrix:** Web assets bundle with specific native builds. If updated remotely in the future, web must verify native Protocol Version.
...
- Approved native inference provider integration testing.
...
- **Initial Feasibility Recommendation:** Rent a Cloud Mac or use a temporary local Mac to validate the iOS packaging spike and inference.
...
- **Simulator-only testing:** Rejected. Native inference and real memory limits require physical iPhones.
