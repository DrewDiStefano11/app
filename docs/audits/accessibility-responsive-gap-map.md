...
- **Safe-area:** Bottom navigation safe-area handling is confirmed implemented, but other fixed surfaces may pose safe-area risks requiring verification.
- **Overlays:** Custom `BottomSheet` and `ConfirmDialog` lack Escape key dismissal and explicit focus traps (Confirmed missing).
...
- **Why browser behavior is not claimed:** This is a static analysis audit. Findings about visual clipping, screen reader announcements, or virtual keyboard interactions are derived purely from code and must be verified in a real environment.
- **Classification system:**
  ...
  - **Confirmed implemented:** Bottom navigation explicitly uses `env(safe-area-inset-bottom)`. Risk is isolated to other surfaces.
...
- _Safe-area overlap_ -> Bottom navigation is confirmed implemented; other overlays require browser verification.
- _Escape dismissal_ -> Custom `BottomSheet` and `ConfirmDialog` Escape dismissal are confirmed missing. Backdrop and close button dismissal are confirmed implemented.
...
- **Confirmed implemented:** Bottom navigation (`src/components/app/bottom-nav.tsx`) explicitly uses `pb-[max(10px,env(safe-area-inset-bottom))]`.
- **Requires browser verification:** Bottom sheets (`src/components/app/sheet.tsx`) have hardcoded heights (`max-h-[88dvh]`) that may interact poorly with top/bottom safe areas. Other surfaces (Jarvis composer, fixed action buttons) require verification.
...
- **Accessibility Tests:** `tests/e2e/keyboard-focus-accessibility-smoke.spec.ts` verifies basic tabbing. Does not test screen reader output or ARIA roles.
...
- [ ] Escape dismissal is added and consistent across all custom popups and sheets.
