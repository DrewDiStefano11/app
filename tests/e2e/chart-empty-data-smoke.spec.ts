...
test.describe("Progress Chart Empty Data Smoke", () => {
  test("Scenario A - Progress renders safely with completely empty logged data arrays", async ({
    page,
  }) => {
    ...
    await expect(
      page
        .getByText(/Not enough data|No training volume|No photos yet|No weigh-ins|No goals/i)
        .first(),
    ).toBeVisible();
  });
  ...
  test("Scenario B - Progress renders safely with only bodyweight data", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "Bodyweight vs time", exact: true }).first(),
    ).toBeVisible();
  });
  ...
  test("Scenario C - Progress renders safely with only workout data", async ({ page }) => {
    ...
    // App should still render safely despite other arrays being empty
    await expect(
      page.getByText(/Not enough data|No weigh-ins|No goals|Training Vol Δ/i).first(),
    ).toBeVisible();
  });
