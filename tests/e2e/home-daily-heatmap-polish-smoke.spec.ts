...
test.describe("Home Daily Heatmap Polish", () => {
  test("heatmap card polish, interactions, and mode descriptions", async ({ page }) => {
    ...
    await expect(heatmapSheet.getByText("Front", { exact: true })).toBeVisible();
    await expect(heatmapSheet.getByText("Back", { exact: true })).toBeVisible();

    // 11 & 12. Muscle interaction and safe close
    const chestRegion = heatmapSheet.locator('.body-heatmap-region[aria-label*="Chest"]');
