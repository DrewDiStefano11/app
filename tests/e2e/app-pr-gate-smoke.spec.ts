...
test.describe('App PR Gate Smoke', () => {
  test('broad app load, navigation, and quick actions', async ({ page }) => {
    ...
    for (const action of quickActions) {
      ...
      const overlay = page.getByRole('heading', { name: action.headingName, exact: true }).locator('xpath=..');
      ...
      await overlay.getByRole('button').filter({ has: page.locator('svg') }).first().click();
