...
test.describe('Progress Rich Data Smoke Test', () => {
  ...
  test('should render Progress/Stats safely with rich data and handle navigation/reloads', async ({ page }) => {
    ...
    await page.evaluate(({ timestamp, oneDayInMs }) => {
      localStorage.setItem("fitcore.v1", JSON.stringify({
        ...
        goals: [
          { id: "g1", type: "weekly_workouts", label: "Train 5x per week", target: 5, current: 1 },
        ],
        bodyweightEntries: [
          { id: "bw1", weightLb: 178, date: timestamp - oneDayInMs * 2, createdAt: timestamp - oneDayInMs * 2 },
          { id: "bw2", weightLb: 180, date: timestamp - oneDayInMs, createdAt: timestamp - oneDayInMs },
        ],
        workouts: [
          {
            id: "w1",
