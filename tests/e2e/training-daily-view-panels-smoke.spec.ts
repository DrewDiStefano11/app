...
test.describe('Training Daily View Smoke', () => {
  ...
  test('scenario coverage for Training Daily View layout and active workout paths', async ({ page }) => {
    ...
    // However, pressing escape is safer and highly robust as a fallback for playwright strict mode violations with overlaps
    await page.locator('.sheet-header').getByRole('button').first().click();
