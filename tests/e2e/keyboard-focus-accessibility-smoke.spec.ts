  ...
  test("main shell, navigation, and overlays are keyboard accessible", async ({ page }) => {
    ...
    const checkFatalErrors = async () => {
      if (pageErrors.length > 0) {
        throw new Error(
          `Fatal page errors occurred: ${pageErrors.map((e) => e.message).join(", ")}`,
        );
      }
    ...
    const sections = [
      ...
      { name: "Progress", navButton: "Stats", heading: "Progress", exactHeading: true },
    ];

    for (const section of sections) {
      await expect(
        page.getByRole("button", { name: section.navButton, exact: section.exactNav }),
      ).toBeVisible();
    }
