...
async function seedReloadableOnboardedState(page: Page) {
  ...
  // Seed with realistic meal entry
  await page.evaluate(
    ({ key, version }) => {
      localStorage.clear();

      const now = new Date();
      // Ensure the meal is logged "today" in local time to avoid timezone boundary issues
...
test.describe("Nutrition Daily View panels smoke", () => {
  ...
  test("scenario coverage for Nutrition/Fuel Daily View layout and panels", async ({ page }) => {
    ...
    const logMealSheet = page.getByRole("heading", { name: "Log Meal", exact: true })
    ...
    if (await cancelBtn.isVisible()) {
      ...
      await page.locator('.sheet-backdrop').first().click({ force: true, position: { x: 10, y: 10 } });
    }
    ...
    await expect(page.locator('nav').getByRole('button', { name: 'Fuel', exact: true }).or(page.getByRole('button', { name: /Expand navigation/ }))).toBeVisible();
