...
test.describe("Date Boundary and Today Rollup Smoke Test", () => {
  test("mixed-date persisted state does not crash the app", async ({ page }) => {
    ...
    await expect(
      page
        .getByRole("heading", { name: "Nutrition", exact: true })
        .or(page.getByText("FitCore Score", { exact: true })),
    ).toBeVisible();
    await checkFatalTexts(page);
    ...
    await expect
      .poll(async () => {
        const storageState = await readPersistedFitCoreState(page);
        ...
        const hasWorkoutToday = workouts.some((w) => w.id === "workout-today");
        const hasWorkoutYesterday = workouts.some(
          (w) => w.id === "workout-yesterday" || w.id === "workout-older",
        );

        const hasMealToday = mealEntries.some((m) => m.id === "meal-today");
        const hasMealYesterday = mealEntries.some(
          (m) => m.id === "meal-yesterday" || m.id === "meal-older",
        );

        const hasWeightToday = bodyweightEntries.some((b) => b.id === "weight-today");
        const hasWeightYesterday = bodyweightEntries.some(
          (b) => b.id === "weight-yesterday" || b.id === "weight-older",
        );
