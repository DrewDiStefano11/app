...
describe("Spoken Number Normalization", () => {
  ...
  describe("Incomplete and rejected constructions", () => {
    it("leaves incomplete or invalid constructions unchanged", () => {
      assert.equal(normalizeSpokenNumbers("one hundred and").normalizedText, "one hundred and");
      assert.equal(normalizeSpokenNumbers("one thousand and").normalizedText, "one thousand and");
      assert.equal(normalizeSpokenNumbers("twenty-").normalizedText, "twenty-");
    ...
    it("preserves multiple mixed valid and invalid replacements", () => {
      assert.equal(
        normalizeSpokenNumbers("one hundred and five reps with twenty- then twenty-one")
          .normalizedText,
        "105 reps with twenty- then 21",
      );
    });
  });
